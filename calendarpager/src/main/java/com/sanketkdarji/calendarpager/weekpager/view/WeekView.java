...
public class WeekView extends View {
  ...
  private boolean mLastPositionFinishing;
  ...
  protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < DAY_IN_WEEK; i++) {
      ...
      if (mDaysPosition % DAY_IN_WEEK == i && mDaysPosition / DAY_IN_WEEK == mWeekPosition) {
  ...
  private void drawSpringView(Canvas canvas) {
    if (mWeekPosition != mDaysPosition / DAY_IN_WEEK) return;
    if (mLastPositionFinishing) {
  ...
  public void onViewPageScroll(int position, float positionOffset, int positionOffsetPixels) {
    ...
    if (position % DAY_IN_WEEK == 6 && positionOffset > LAST_OFFSET_IN_WEEK) {
      mLastPositionFinishing = true;
      mSpringView.paint.setAlpha(0);
    } else {
      mLastPositionFinishing = false;
      mSpringView.paint.setAlpha(255);
    }
  ...
  private void createWeekCalendarDays() {
    ...
    calendar.roll(Calendar.DAY_OF_YEAR, mWeekPosition * DAY_IN_WEEK);
    for (int i = 0; i < DAY_IN_WEEK; i++) {
  ...
  @Override public boolean onTouchEvent(MotionEvent event) {
    if (event.getAction() == MotionEvent.ACTION_UP) {
      ...
      if (calendarDay != null) {
        int position = mWeekPosition * DAY_IN_WEEK + getPositionFromLocation(event.getX(),
            event.getY());
  ...
  public void setSelectPosition(int selectPosition) {
    mDaysPosition = selectPosition;
    invalidate();
  }

  public void setAbleDates(ArrayList<CalendarDay> ableCalendarDays) {
    mAbleDates.clear();
    for (CalendarDay calendarDay : ableCalendarDays) {
